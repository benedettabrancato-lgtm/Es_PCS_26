...
int main() {
    ...
    for (int i = 0; i<100; i++){ //itero sul vettore e ordino
        ...
        quicksort(casuali,0,casuali.size()-1);
        //verifico corretto funzionamento
        if(!is_sorted(casuali.begin(),casuali.end())){
            std::cerr<<"Il quicksort non ha ordinato correttamente il vettore\n";
            return EXIT_FAILURE;
        }
    }
    ...
    if (!is_sorted(stringa.begin(),stringa.end())){
            std::cerr<<"Il quicksort non ha ordinato correttamente il vettore di stringhe \n";
            return EXIT_FAILURE;
