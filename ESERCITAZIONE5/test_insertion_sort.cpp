...
int main() {
    ...
    for (int i = 0; i<100; i++){ //itero sul vettore e ordino
        ...
        insertion_sort(casuali); //ordino il vettore

        //verifico corretto funzionamento
        if(!is_sorted(casuali)){
            std::cerr<<"L'Insertion sort non ha ordinato correttamente il vettore\n";
            return EXIT_FAILURE;
        }
    }
    ...
    if (!is_sorted(stringa.begin(),stringa.end())){
        std::cerr<<"L'insertion sort non ha ordinato correttamente il vettore di stringhe \n";
        return EXIT_FAILURE;   
    }
    return EXIT_SUCCESS;
}
